        ...
        <div className="saludHome">
            ...
            <div className="containerSaludHome">
                {salud.map((item, index) => (
                    <div key={index} className={item.idn === 2 ? `cont` : item.idn === 1 ? `contTwoEspacio` : `contTwo`}>
                        <a className="links" href={`https://www.fitknow.fit/${item.id}`}>
                        <img className={item.idn === 2 ? `imgSaludHomeTwo` : `imgSaludHome`} src={item.img} alt={item.alt} loading="lazy" />
